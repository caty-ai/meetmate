...
describe("Exit immediate detection", () => {
  ...
  describe("detectExitIntent()", () => {
    ...
    it("detects exact match '退出して'", () => {
      assert.equal(detectExitIntent("退出して", agentProfile), true);
    });

    it("detects exact match '退室して'", () => {
      assert.equal(detectExitIntent("退室して", agentProfile), true);
    });

    it("detects '退出していいよ'", () => {
      assert.equal(detectExitIntent("退出していいよ", agentProfile), true);
    });

    it("detects '退室していいよ'", () => {
      assert.equal(detectExitIntent("退室していいよ", agentProfile), true);
    });

    it("detects with extra text 'それでは退出していいよ'", () => {
      assert.equal(detectExitIntent("それでは退出していいよ", agentProfile), true);
    });

    it("detects with extra text 'もう退室して大丈夫'", () => {
      assert.equal(detectExitIntent("もう退室して大丈夫", agentProfile), true);
    });

    it("detects '退出口' (includes 退出)", () => {
      // '退出口' contains '退出' — current includes() behavior matches
      assert.equal(detectExitIntent("退出口", agentProfile), true);
    });

    it("detects '退室する予定' (includes 退室)", () => {
      // '退室する予定' contains '退室' — current includes() behavior matches
      assert.equal(detectExitIntent("退室する予定", agentProfile), true);
    });

    it("does NOT detect 'こんにちは' (non-exit)", () => {
      assert.equal(detectExitIntent("こんにちは", agentProfile), false);
    });

    it("does NOT detect '今日の天気' (non-exit)", () => {
      assert.equal(detectExitIntent("今日の天気", agentProfile), false);
    });

    it("detects with agent-specific exit command", () => {
      const customAgentProfile = { exitCommands: ["バイバイ", "さようなら"] };
      assert.equal(detectExitIntent("バイバイ", customAgentProfile), true);
      assert.equal(detectExitIntent("さようなら", customAgentProfile), true);
      assert.equal(detectExitIntent("退出して", customAgentProfile), false); // default not included
    });
    ...
    it("does NOT detect pure katakana 'タイシュツシテ' (different script)", () => {
      ...
      assert.equal(detectExitIntent("タイシュツシテ", agentProfile), false);
    });
    ...
    it("normalizes long vowel marks in custom commands", () => {
      ...
      assert.equal(detectExitIntent("バイバイ", customProfile), true);
    });
  });
});