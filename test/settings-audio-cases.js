...
function multipart({ metadata, audio = MP3, filename = "clip.mp3", audioType = "audio/mpeg", extraParts = [] }, boundary = "meetmate-audio-boundary") {
  const parts = [
    {
      headers: [
...
test("T12-09 upload streams multipart, converts with an argument array, projects views, stales, and deletes the owned pair", async (t) => {
  const calls = [];
  const setup = fixture(t, baseConfig(), { spawnFn: fakeSpawn({ capture: (call) => calls.push(call) }) });
  ...
  assert.equal(envelope.fields.audio_clips[0].stale, true);
...
test("T12-09 a real generated MP3 converts to mono S16LE at the effective sample rate when ffmpeg is available", async (t) => {
  ...
  if (available.status !== 0) {
    ...
    assert.equal(JSON.parse(res.body).clip.sampleRate, 24_000);
    return;
  }
...
test("T12-09 resolved-home storage never mutates bundled assets and rejects a managed-directory symlink", async (t) => {
  ...
  const before = fs.statSync(packageAssets).mtimeMs;
  const setup = fixture(t);
  ...
  assert.equal(fs.statSync(packageAssets).mtimeMs, before);
...
test("T12-09 the real pipeline forwards the fixed phrase role to the cache boundary", async () => {
  ...
  require.cache[require.resolve(path.join(src, "llm-provider.js"))] = moduleEntry(path.join(src, "llm-provider.js"), {
    createLlmProvider: () => ({ name: "openclaw", streamChat: async function* () {}, VOICE_SYSTEM_ADDENDUM: "", buildVoiceAddendum: () => "" }),
  });
  ...
  try {
    ...
    const pipeline = createPipeline(
      ...
      () => {},
      {
        ...
        ackVariants: ["了解です"], echoCooldownMs: 1, greeting: "", exitDetection: false,
      },
      { agents: { caty: { wakeWords: ["ケイティ"] } }, selectedAgentIds: ["caty"], defaultAgentId: "caty" },
    );
    stt.emit("utterance_end", "ケイティ、確認して");
    await waitUntil(() => roles.includes("ack"));
    pipeline.close();
    assert.deepEqual(roles, ["ack"]);
  } finally {
...
test("T12-09 pending restart TTS edits mark new runtime-format uploads stale and block automatic lookup", async (t) => {
  const setup = fixture(t);
  const changed = await invoke(setup.handler, jsonRequest("PUT", "/api/settings", {
    schemaVersion: 1,
